      ...
      cartas.innerHTML += `
            <div class="card my-3 col-md-6 col-lg-3 col-sm-2 col-xs-2">
              <div class="card text-center" >
                ...
                <div class="card-body">
                  <h6 class="card-title">${element.nombre.toUpperCase()} ${
        element.tipo
      }</h6>
                  <a id="${element.tipo}" class="btn btn-primary ${
        element.nombre
      }" onclick=''>${element.precio}</a>
                </div>
              </div>
            </div>`;
    });
  ...
  agregarCarrito() {
    ...
    netflix.forEach((element) => {
      element.addEventListener("click", () => {
        ...
        user.addProduct("Netflix", element.id, element.innerText);
    ...
    plex.forEach((element) => {
      element.addEventListener("click", () => {
        ...
        console.log(user.getCurrentUser());
        console.log(element);
        user.addProduct("Plex", element.id, element.innerText);
    ...
    HBO.forEach((element) => {
      element.addEventListener("click", () => {
        ...
        console.log(user.getCurrentUser());
        console.log(element);
        user.addProduct("HBO", element.id, element.innerText);
        const toastLiveExample = document.getElementById("liveToast");
        const toast = new bootstrap.Toast(toastLiveExample);
        let datos = document.querySelector(".toast-body");
    ...
    Disney.forEach((element) => {
      element.addEventListener("click", () => {
        ...
        console.log(user.getCurrentUser());
        console.log(element);
        user.addProduct("Disney", element.id, element.innerText);
    ...
    crunchyrol.forEach((element) => {
      element.addEventListener("click", () => {
        ...
        console.log(user.getCurrentUser());
        console.log(element);
        user.addProduct("Crunchyroll", element.id, element.innerText);
    ...
    primevideo.forEach((element) => {
      element.addEventListener("click", () => {
        ...
        console.log(user.getCurrentUser());
        console.log(element);
        user.addProduct("Prime Video", element.id, element.innerText);
        const toastLiveExample = document.getElementById("liveToast");
        const toast = new bootstrap.Toast(toastLiveExample);
        let datos = document.querySelector(".toast-body");
